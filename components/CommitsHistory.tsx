...
import { useEffect, useRef, useState, cloneElement, Suspense } from 'react';
...
export default function CommitsHistory() {
  ...
  function handleLinkClick() {
    posthog.capture('github_link_clicked', { location: 'commits_graph' });
  }

  return (
    <div className="w-fit max-w-full mx-auto mt-8">
      <div className="flex items-center justify-between flex-wrap gap-y-1 gap-x-2 mb-4">
        <h3 className="text-xl font-medium tracking-wider">
          {totalCount || 'Histórico de'} Commits no GitHub
        </h3>
        <a
          href="https://github.com/iagobruno"
          target="_blank"
          onClick={handleLinkClick}
          className="text-sm flex items-center gap-1.5 ml-auto"
        >
        ...
        <Suspense fallback={null}>
          <GitHubCalendar
            ...
            transformData={(contributions) => {
              if (!totalCount)
                setTotalCount(contributions.reduce((acc, curr) => acc + curr.count, 0));
