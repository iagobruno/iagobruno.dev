...
export default function Button({ children, className, growOnHover = true, ...props }: Props) {
  ...
  useMount(() => {
    const strength = 0.2;
    const btnElement = containerRef.current!.querySelector<HTMLElement>('.group\\/button')!;
    const zoneElement = containerRef.current!.querySelector<HTMLElement>('.zone')!;
    const btnWidth = btnElement.getBoundingClientRect().width + 4;

    containerRef.current!.style.width = btnWidth + 'px';
    containerRef.current!.style.height = btnElement.getBoundingClientRect().height + 'px';
      ...
      <div className="zone inline-block">
        <div className="group/button inline-block">
          <a
            {...props}
            className={cn(
              'inline-block bg-primary text-white! text-sm md:text-base leading-6.5 uppercase tracking-wide rounded-full px-6 cursor-pointer no-underline! overflow-hidden transition-all duration-300 [-webkit-tap-highlight-color:transparent] hover:shadow-lg active:scale-95',
              {
                'hover:rotate-0 hover:scale-110 dark:hover:bg-white dark:hover:!text-black':
                  growOnHover,
              },
              className,
            )}
